...
- **Playlists:** the speaker returns Sonos playlists as `<container>` elements, not `<item>`, so the first parser saw none. Fixed from a real capture (`testdata/browse-playlists-one.xml`). Enqueuing a playlist with empty metadata was verified on Dining Room: the speaker replaced the queue with the playlist's tracks. Playing it from Alfred was confirmed in acceptance. The "Playlists empty" notes in S1 and S2 were true of that day's household.
- **Key hints:** favorite and playlist rows say what each key does ("↩ play · ⌘↩ add to end · ⌥↩ play next"). The volume row already had one. The now-playing row keeps artist and album as its subtitle, so its hint is a per-modifier subtitle: holding ⌘ shows "Next track", holding ⌥ shows "Previous track" (`mods.<key>.subtitle` in the JSON). Not yet seen in Alfred; if it doesn't show, the row simply has no hint.
...
- **Streams and AirPlay:** while AirPlay plays, the speaker reports `NOT_IMPLEMENTED` as the track metadata. The first build failed the whole refresh on it, and the row said "Can't reach Sonos". `NowPlaying` now accepts metadata with no XML in it (fixture `testdata/getpositioninfo-airplay.xml`), and a loaded track with no title shows as "Playing".
- **Failure reason:** the last refresh's error is cached and shown as the "Can't reach Sonos" row's subtitle ("Couldn't read the speakers: …"), because any failed step, not only an unreachable speaker, produces that row.
- **Shuffle and repeat on streams:** the speaker answers a play-mode change with UPnP error 712 while a radio stream is the source (verified on Dining Room; AirPlay not tested). The Shuffle and Repeat rows are hidden while a track is loaded that isn't playing from the queue.
- **Fresh binaries stall:** a newly built binary can time out on its first network calls for a minute or two, then run normally. Cause unverified. Retry before treating it as a bug (also noted in `AGENTS.md`).
- **Acceptance (2026-09-21):** the checklist passed in Alfred: rows, filtering, room choice, key hints, playlist and favorite playback, add and play-next, transport, volume, shuffle and repeat (on the queue), sleep timer, queue jump, error notification.
